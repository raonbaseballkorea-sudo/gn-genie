...
function extractOrderJson(text: string): string | null {
  ...
  for (const pattern of patterns) {
    ...
    if (match) {
      ...
      devLog('[ROUTE] ORDER_COMPLETE found at index:', searchFrom, 'pattern:', pattern);
      break;
    }
  }
  ...
  if (jsonStart === -1) {
    devLog('[ROUTE] No JSON start found');
    return null;
  }

  // 중괄호 depth 추적으로 JSON 끝 찾기 (문자열 내부의 중괄호는 무시)
  let depth = 0;
  let jsonEnd = -1;
  let inString = false;
  let escaped = false;
  ...
  if (jsonEnd === -1) {
    devLog('[ROUTE] No JSON end found');
    return null;
  }
...
export async function POST(req: NextRequest) {
  ...
  if (imageBase64 && !ALLOWED_IMAGE_TYPES.includes(imageType)) {
  ...
  devLog('[ROUTE] Response includes ORDER_COMPLETE:', rawText.includes('ORDER_COMPLETE'));
  devLog('[ROUTE] Response tail (last 200 chars):', rawText.slice(-200));

  const rawJson = extractOrderJson(rawText);

  if (rawJson) {
    devLog('[ROUTE] Extracted rawJson (first 300):', rawJson.substring(0, 300));

    try {
      const cleanJson = sanitizeJsonString(rawJson);
      devLog('[ROUTE] cleanJson (first 300):', cleanJson.substring(0, 300));

      const parsed = JSON.parse(cleanJson);

      // order_type 필드로 실제 주문 JSON인지 확인
      if (!parsed.order_type) {
        devLog('[ROUTE] No order_type field — not a valid order JSON');
